...
struct BaseNetworking: NetworkingType {
    ...
    static func defaultNetworking() -> Self {
        return BaseNetworking(provider: newProvider(plugins))
    }

    static func stubbingNetworking() -> Self {
        return BaseNetworking(provider: OnlineProvider(endpointClosure: endpointsClosure(), requestClosure: BaseNetworking.endpointResolver(), stubClosure: MoyaProvider.immediatelyStub, online: .just(true)))
    }
...
private func newProvider<T>(_ plugins: [PluginType], xAccessToken: String? = nil) -> OnlineProvider<T> where T: ProductAPIType {
    return OnlineProvider(endpointClosure: BaseNetworking.endpointsClosure(xAccessToken),
                          requestClosure: BaseNetworking.endpointResolver(),
                          stubClosure: BaseNetworking.APIKeysBasedStubBehaviour,
                          plugins: plugins)
}
